...
---
과제 영상링크: https://drive.google.com/file/d/17G5yDDcXg9nyfccjL8iUlovQ0Rp7bbnl/view?usp=sharing
<br>- 영상 화질이 깨져서 안 보인다면 다운로드 후 보는것을 추천합니다. 
## 1. 개요 (Overview)
본 과제는 ATmega128 마이크로컨트롤러를 활용하여 주요 주변장치(LED, 스위치, UART)를 제어하는 것을 목표로 함.

### 핵심 목표
* ATmega128를 통한 LED, 스위치, UART제어
* 스위치를 이용해서 LED, UART값 제어
---
...
### 동작 시나리오
1. SW1 누르면 모든 상태 초기화 후, “RESET” 문자열 PC로 전송
2. PC에서 0~7 숫자 입력 시 해당 번호의 LED 켜기 (예외 처리 필요, PC로 “n LED on” 문자열 전송)
...
| :--- | :--- | :--- |
| **Claude** | 코드 디버깅 / UART 학습 | - 빌드 에러 및 문법 오류 원인 분석<br>- 레지스터, 마이크로컨트롤러, UART에 관한 예제코드 분석 |
